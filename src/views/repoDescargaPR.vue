...
export default {
  ...
  methods: {
    ...
    csvExport() {
      ...
      const csv = json2csvParser.parse(this.repoListPR);
      ...
      comments.forEach(comment => {
        try{
          ...
          row = {
            ...
            cohesionGrupal: estadisticaPR.cohesionGrupal,
            colaboracionGrupal: estadisticaPR.colaboracionGrupal,
            mimicaGrupal: estadisticaPR.mimicaGrupal,
            tonoGrupal: estadisticaPR.tonoGrupal,
    ...
    getParticipantsData(listaParticipantes) {
      ...
      if(listaParticipantes.length != 0){
        ...
        //LLamo a función para armar lista de comentarios
        this.ListCommentsRow();
        //LLamo a función para armar lista de PRs en base a cada comentario
        this.createSummaryPR();
        this.show = !this.show;
        this.loading = !this.loading;
      }
    },
    createSummaryPR() {
      ...
      this.repoListComments.forEach(comment => {
        if ((lastCommentNumber != comment.PR) && (lastCommentNumber != 0)) {
          this.repoListPR.push(PRSummary);
          PRSummary = {
            Repository: "",
            RepoTotalFollowers: "",
            RepoTotalStargazers: "",
        ...
        PRSummary = {
            ...
            RepoTotalFollowers: comment.RepoTotalFollowers,
            RepoTotalStargazers: comment.RepoTotalStargazers,
            RepoTotalWatchers: comment.RepoTotalWatchers,
            RepoTotalForks: comment.RepoTotalForks,
            PR: comment.PR,
            PRAutor: comment.PRAutor,
            cantParticipantes: comment.cantParticipantes,
            estado: comment.estado,
            duracionDias: comment.duracionDias,
            lineasModif: comment.lineasModif,

            cohesionGrupal: comment.cohesionGrupal,
            colaboracionGrupal: comment.colaboracionGrupal,
            mimicaGrupal: comment.mimicaGrupal,
            tonoGrupal: comment.tonoGrupal,

            comentarios: PRSummary.comentarios + "\n" + comment.comentario,
            Thumbs_Up: PRSummary.Thumbs_Up + comment.Thumbs_Up,
            Thumbs_Down: PRSummary.Thumbs_Down + comment.Thumbs_Down,
            Laugh: PRSummary.Laugh + comment.Laugh,
            Hooray: PRSummary.Hooray + comment.Hooray,
            Confused: PRSummary.Confused + comment.Confused,
            Heart: PRSummary.Heart + comment.Heart,
            Rocket: PRSummary.Rocket + comment.Rocket,
            Eyes: PRSummary.Eyes + comment.Eyes,
          }

        lastCommentNumber = comment.PR;
      });
